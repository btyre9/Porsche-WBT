(function () {
  ...
  var scorm = {
    ...
    reportFinal: function (scorePercent, threshold) {
      ...
      this.commit();
      // terminate() is called on beforeunload so the learner can continue reviewing
      return true;
    },
    ...
    getLocation: function () {
      ...
      try {
        return this.version === "2004"
          ...
          : this.api.LMSGetValue("cmi.core.lesson_location");
      } catch (_e) { return null; }
    },
    ...
    setLocation: function (val) {
      ...
      try {
        return this.version === "2004"
          ? this.api.SetValue("cmi.location", String(val)) === "true"
          : this.api.LMSSetValue("cmi.core.lesson_location", String(val)) === "true";
      } catch (_e) { return false; }
    },
    ...
    setIncomplete: function () {
      ...
      try {
        if (this.version === "2004") {
          var s = this.api.GetValue("cmi.completion_status");
          if (s !== "completed") {
            this.setValue("cmi.completion_status", "incomplete");
            this.commit();
          }
        } else {
          var s2 = this.api.LMSGetValue("cmi.core.lesson_status");
          if (s2 !== "passed" && s2 !== "completed") {
  ...
  var state = {
    ...
    finalAnswered: 0,
    finalTotal: 0,
    activeFQIds: [],       // 5 randomly selected FQ slide IDs for current attempt
    furthestSlide: 0,      // highest slide index reached (for sequential TOC locking)
    quizCompleted: false,  // true after first quiz attempt; unlocks full TOC
    cueEditor: {
      open: false,
      cues: [],
      slideId: "",
  ...
  function injectFQCounter(frameDoc) {
    ...
    if (!slide || !isFQSlide(slide.id) || slide.id === "FQ-CC01-SCORE") return;
    if (!frameDoc) return;
  ...
  function showSlide(i, forceReplay) {
    ...
    state.slideIndex = i;
    // Track how far the learner has progressed (drives TOC locking)
    if (i > state.furthestSlide) {
      state.furthestSlide = i;
      updateTocLock();
    }
    // Save bookmark so learner can resume where they left off
  ...
  function renderToc() {
    ...
    slides.forEach(function (slide, index) {
      ...
      button.type = "button";
      var classes = "toc-item";
      if (isKC || isFQ) classes += " toc-item--indented";
      ...
      button.textContent = displayNum + ". " + title;
      button.addEventListener("click", (function (idx, fqEntry) {
        return function () {
          if (fqEntry) {
            // Re-randomize and jump to first question in shuffled order
            initFinalQuiz();
            var tocSlides = state.data && state.data.slides || [];
            var tocFirstIdx = 0;
            for (var tfi = 0; tfi < tocSlides.length; tfi++) {
              if (tocSlides[tfi].id === state.activeFQIds[0]) { tocFirstIdx = tfi; break; }
            }
            showSlide(tocFirstIdx);
          } else {
            showSlide(idx);
          }
          closeMenu();
  ...
  function updateTocLock() {
    ...
    for (var i = 0; i < items.length; i += 1) {
      ...
      // "visited" class: used by SCORM player CSS — items are locked/dimmed by
      // default; adding "visited" enables them visually and restores pointer events.
      if (unlocked) {
  ...
  function initFinalQuiz() {
    // Collect all FQ question slide IDs (exclude SCORE slide)
    var slides = state.data && state.data.slides || [];
    var allFQIds = [];
    slides.forEach(function (s) {
      if (isFQSlide(s.id) && s.id !== 'FQ-CC01-SCORE') allFQIds.push(s.id);
    });
    // Randomly pick 5 questions in shuffled order (displayed in that random order)
  ...
  // Re-randomizes FQ if we're crossing from non-FQ → FQ, then returns next slide index.
  // Use this instead of getNextSlideIndex whenever the learner explicitly advances.
  function advanceNext(fromIdx) {
    var allSlides = state.data && state.data.slides || [];
    var nextI = getNextSlideIndex(fromIdx);
    var cur = allSlides[fromIdx];
    var nxt = allSlides[nextI];
    if (nxt && isFQSlide(nxt.id) && nxt.id !== 'FQ-CC01-SCORE' &&
        cur && !isFQSlide(cur.id)) {
      initFinalQuiz();
      // Jump directly to first question in the new shuffled order
      var firstId = state.activeFQIds[0];
      for (var fi = 0; fi < allSlides.length; fi++) {
        if (allSlides[fi].id === firstId) { nextI = fi; break; }
      }
    }
    return nextI;
  }

  // Returns the next slide index to show, following activeFQIds order within the quiz
  function getNextSlideIndex(fromIndex) {
    var slides = state.data && state.data.slides || [];

    // When leaving an active FQ question, follow activeFQIds order (not deck order)
    var fromSlide = slides[fromIndex];
    if (fromSlide && isFQSlide(fromSlide.id) && fromSlide.id !== 'FQ-CC01-SCORE') {
      var pos = state.activeFQIds.indexOf(fromSlide.id);
      if (pos >= 0) {
        if (pos < state.activeFQIds.length - 1) {
          // Next question in shuffled order
          var nextId = state.activeFQIds[pos + 1];
          for (var fi = 0; fi < slides.length; fi++) {
            if (slides[fi].id === nextId) return fi;
          }
        } else {
          // Last question — go to SCORE
          for (var si = 0; si < slides.length; si++) {
            if (slides[si].id === 'FQ-CC01-SCORE') return si;
          }
        }
      }
    }

    // Standard linear navigation (skips non-selected FQ questions)
    var nextI = fromIndex + 1;
    while (nextI < slides.length) {
      var s = slides[nextI];
      if (isFQSlide(s.id) && s.id !== 'FQ-CC01-SCORE') {
        if (state.activeFQIds.indexOf(s.id) >= 0) return nextI; // active question
        nextI++; // skip non-selected question
      } else {
        return nextI; // non-FQ slide — always show
      }
    }
    return nextI;
  }
  ...
  async function init() {
    ...
    // Dev mode — set before renderToc so TOC lock check sees the flag
      ...
      switch (e.data.type) {
        ...
        case "sandbox-next":
          if (typeof e.data.correct === 'boolean') {
            ...
            if (curSlide && isFQSlide(curSlide.id) && curSlide.id !== 'FQ-CC01-SCORE') {
              state.finalAnswered += 1;
              if (e.data.correct) state.finalCorrect += 1;
              // Report to SCORM when the last question is answered
              if (state.finalAnswered >= state.finalTotal && state.finalTotal > 0) {
                var fqScore = pct(state.finalCorrect, state.finalTotal);
                var fqThreshold = Number(
                ...
                scorm.reportFinal(fqScore, fqThreshold);
              }
            }
          }
          var snNext = advanceNext(state.slideIndex);
        ...
        case "sandbox-review-module":
          // Learner clicked "Review Module" after quiz — unlock all TOC items and open menu
    ...
    $("btn-next").addEventListener("click", function () {
      ...
      showSlide(advanceNext(state.slideIndex));
    });
    ...
    // Dev mode setup (state.devMode already set above, before renderToc)
    if (state.devMode) {
      ...
      // Auto-open menu so all slides are accessible for testing
      openMenu();
    }

    // Mark session as in-progress (won't overwrite a prior "completed" status)
    scorm.setIncomplete();

    // Restore bookmark — resume where learner left off
    ...
    if (bookmark) {
      ...
      for (var bi = 0; bi < allSlides.length; bi++) {
        if (allSlides[bi].id === bookmark) {
          if (bookmark === 'FQ-CC01-SCORE') {
            // Completed the quiz — unlock all slides
            startIndex = bi;
            state.furthestSlide = allSlides.length - 1;
            state.quizCompleted = true;
          } else if (isFQSlide(bookmark)) {
            // Mid-quiz bookmark: restart quiz from first FQ question slide
            for (var fi = 0; fi < allSlides.length; fi++) {
              if (isFQSlide(allSlides[fi].id) && allSlides[fi].id !== 'FQ-CC01-SCORE') {
                startIndex = fi;
                break;
              }
            }
