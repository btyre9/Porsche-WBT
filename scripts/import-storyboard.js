 ...
 *   >> When user clicks Appearance card → SLD-CC01-001-CLICK-Appearance.mp3
 ...
 *
 * Generated VO file name pattern:
 *   SLD-<CourseID>-<SlideNum>-<TriggerType>[-<Label>].mp3
 *   e.g. SLD-CC01-001-INTRO.mp3 | SLD-CC01-004-CLICK-Appearance.mp3
 */
...
function parseLines(lines) {
  ...
  let current        = null;
  let lastKey        = null;
  let seenFirstSlide = false;   // ignore KV pairs before the first slide heading
  ...
  for (const text of lines) {
    ...
    if (SLIDE_HEADING_RE.test(headingText)) {
      flush();
      seenFirstSlide = true;
      current = { section_heading: headingText };
      continue;
    }

    if (!seenFirstSlide) continue;   // skip intro / metadata before first slide heading
    ...
    if (kv) {
      const key   = canonicalKey(kv[1]);
      if (!key) continue;            // header row (e.g. "Field: Value")
      const value = kv[2].trim();

      if (key === 'Slide-ID' && current && 'Slide-ID' in current) {
        flush();
        current = { section_heading: `Slide${String(slides.length + 1).padStart(2, '0')}` };
      }
  ...
  slides.forEach((slide, idx) => {
    slide['Slide-ID']    = slide['Slide-ID']    || `slide-${String(idx + 1).padStart(2, '0')}`;
...
function parseSlideIdParts(slideId) {
  ...
  // Full format: SLD-CC01-001, KC-CC01-001, FQ-CC01-001
  const full = upper.match(/^([A-Z]+)-([A-Z]{2}\d+)-(\d+)$/);
  if (full) return { prefix: full[1], courseId: full[2], slideNum: full[3].padStart(3, '0') };
  // Bare course+num: CC01-001
  const bare = upper.match(/^([A-Z]{2}\d+)-(\d+)$/);
...
function buildFileName(prefix, courseId, slideNum, triggerType, label) {
  return [prefix, courseId, slideNum, triggerType, ...(label ? [label] : [])].join('-') + '.mp3';
}
...
async function main() {
  ...
  if (slides.length === 0) {
    console.error(
      'No slides found. Ensure your doc has slide headings like "Slide 01" and\n' +
      'two-column tables with Field | Value rows.'
    );
    process.exit(1);
  }
